...
export default async function member_leaderbord(mgs: Message, table: tables) {
    ...
    if (user_list.length == 0) {
        ...
        const user = (await table.user_table.findOne({ where: { userId: mgs.author.id } }))?.get();
        // Set Description
        if (user) embed.setDescription(`<@!${user.userId}> -----> ${user.saldoDepositatoTot}`);
        // Set List        
        for (var x = 0; x < 10; x++) {
            // Get User Data
            ...
            // Add Field
            if (user_data) embed.addField(`--------------------------`, `**${x + 1}:** <@!${user_data.userId}> ----> ${user_data.saldoDepositatoTot}`);
        }
        // Send Embed
        mgs.channel.send(embed);
    }
