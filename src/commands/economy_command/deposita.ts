...
// Export Funcion
export default async function deposita(
  mgs: Message,
  table: tables,
  args: string[]
) {
  ...
  if (!Number(args[2]) || Number(args[2]) < 1000)
    return errorMGS(mgs, "Valore inserito invalido");
  // Get User
  const user = (await getUserDB(mgs.author.id, table))?.get();
  // Check Credits
  if (!user?.saldo || user?.saldo < Number(args[2]))
    return errorMGS(mgs, "Non hai abbastanza soldi");
  // Check Crew
  ...
  await table.user_table.update(
    { saldo: user.saldo - Number(args[2]) },
    { where: { userId: mgs.author.id }, transaction: t }
  );
  // Pay Crew
  await table.crew_table.update(
    { saldo: crew.saldo + Number(args[2]) },
    { where: { crewId: user.ciurmaId }, transaction: t }
  );
  // Commit Transaction
  t.commit()
    .then(() => {
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `${mgs.author.username} hai depositato con successo ${args[2]} talleri.`
        );
      // Send Message
