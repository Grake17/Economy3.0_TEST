...
export default async function giveaway(mgs: Message, table: tables) {
    ...
    // Check User
    if (!user || !user.saldo) return errorMGS(mgs, `L'utente non è sul DB\nUsa il comando **>e registra** per registrarti`);
    // Check Credit
    if (user.saldo < 50000) return errorMGS(mgs, `Non hai abbastanza talleri\n**Saldo corrente**: ${user.saldo}`);
    // Check Roles
    if (mgs.guild?.members.cache.get(mgs.author.id)?.roles.cache.get(roles.role_giveaway)) return errorMGS(mgs, "Sei già iscritto al giveaway!");
    // Make Payment
    table.user_table.update({ saldo: user.saldo - 50000 }, { where: { userId: mgs.author.id } }).then(() => {
        // Send Embed
